...
export const apiService = {
  ...
  async getIptvAccounts() {
    try {
      ...
      if (data) {
        ...
        try {
          localStorage.setItem('g3d_iptv_cuentas', JSON.stringify(mapped));
        } catch {}
        return mapped;
      }
    } catch (e: any) {
  ...
  async saveIptvAccount(account: any) {
    ...
    try {
      // Registrar también en LocalStorage para redundancia local inmediata sin re-consultar la base de datos
      let accounts: any[] = [];
      const local = localStorage.getItem('g3d_iptv_cuentas');
      if (local) {
        try { accounts = JSON.parse(local); } catch { accounts = []; }
      }
  ...
  async deleteIptvAccount(username: string) {
    ...
    try {
      // 1. Eliminar perfiles asociados de forma segura
      try {
        const { data: dbProfiles } = await supabase
          .from('iptv_clientes_perfiles')
          .select('id')
          .or(`username_cuenta.eq.${username},username_cuenta.ilike.${username}`);
        ...
        console.warn("[deleteIptvAccount] Limpieza opcional de perfiles:", profErr);
      }

      // 2. Eliminar sesiones activas asociadas si existieran
      try {
        await supabase
        ...
        console.warn("[deleteIptvAccount] Limpieza opcional de sesiones activas:", sessSupaErr);
      }

      // 3. Eliminar la cuenta del cliente principal
      const { error } = await supabase
        .from('iptv_clientes')
        .delete()
        .or(`username.eq.${username},username.ilike.${username}`);

      if (error) {
        supabaseError = error.message;
        console.warn("[deleteIptvAccount] Error borrando de iptv_clientes:", error.message);
      }

      // 4. Limpieza opcional de imágenes en storage (silenciosa y sin bloquear la respuesta)
      setTimeout(async () => {
        try {
          const buckets = ['IPTV_TRANSFERENSIA', 'IPTV_STORGE', 'public_assets'];
          for (const bucket of buckets) {
            try {
              const { data: filesList } = await supabase.storage.from(bucket).list(username);
      ...
      console.warn("[deleteIptvAccount] Excepción durante eliminación en Supabase:", e.message || e);
    ...
    try {
      ...
      accounts = accounts.filter((it: any) => (it.username || '').toLowerCase() !== lowerUsername);
      localStorage.setItem('g3d_iptv_cuentas', JSON.stringify(accounts));

      // Limpieza cascada local de perfiles
      let profiles = JSON.parse(localStorage.getItem('g3d_iptv_perfiles') || '[]');
      profiles = profiles.filter((p: any) => (p.username_cuenta || '').toLowerCase() !== lowerUsername);
      ...
      sessions = sessions.filter((s: any) => (s.username_cuenta || '').toLowerCase() !== lowerUsername);
  ...
  async getIptvMessageTemplates() {
    try {
      const { data, error } = await supabase
        .from('iptv_plantillas_mensajes')
        .select('*');
      if (error) throw error;
      if (data && data.length > 0) {
        const templatesObj: Record<string, string> = {};
        data.forEach((row: any) => {
          if (row.clave && row.mensaje) {
            templatesObj[row.clave] = row.mensaje;
          }
        });
        localStorage.setItem('g3d_iptv_message_templates', JSON.stringify(templatesObj));
        return templatesObj;
      }
      throw new Error("Sin plantillas en base de datos");
    } catch (e: any) {
      console.warn("[getIptvMessageTemplates] Usando localStorage o fallback:", e.message || e);
      const saved = localStorage.getItem('g3d_iptv_message_templates');
      return saved ? JSON.parse(saved) : null;
    }
  },

  async saveIptvMessageTemplate(clave: string, mensaje: string, titulo?: string) {
    try {
      const payload: any = {
        clave,
        mensaje,
      ...
      console.warn("[saveIptvMessageTemplate] Guardado alternativo local:", e.message || e);
    ...
    try {
      ...
      if (data) {
        ...
        try {
          ...
          if (!plansError && plansData && plansData.length > 0) {
            dbSalePlans = plansData.map(p => {
              const comisionVend = p.comision_vendedor != null ? Number(p.comision_vendedor) : (p.comision != null ? Number(p.comision) : 0);
              const comisionRef = p.comision_referente != null ? Number(p.comision_referente) : 0;
              return {
                id: p.id,
                ...
                comision_vendedor: comisionVend,
                comision_referente: comisionRef,
                categoria_nombre: p.categoria_nombre || '',
                categoria_id: p.categoria_id || 'vip',
                visible: p.visible !== false,
                order: p.order != null ? Number(p.order) : 999,
                custom_override: !!p.custom_override
              };
            });
            // Ordenar los planes de venta según el orden establecido
            dbSalePlans.sort((a, b) => (a.order ?? 999) - (b.order ?? 999));
          } else {
        ...
        const resultObj = {
          ...
          costo_credito: data.costo_credito != null ? Number(data.costo_credito) : 1000.00,
          comision_administrador: data.comision_administrador != null ? Number(data.comision_administrador) : 1500.00,
          comision_vendedor: data.comision_vendedor != null ? Number(data.comision_vendedor) : 800.00,
          comision_referente: data.comision_referente != null ? Number(data.comision_referente) : 200.00,
        ...
        try {
          localStorage.setItem('g3d_iptv_finanzas', JSON.stringify(resultObj));
        } catch {}
        return resultObj;
      }
      throw new Error("No data returned");
  ...
  async saveIptvFinances(data: any) {
    try {
      ...
      const payload = {
        ...
        costo_credito: data.costo_credito !== undefined ? Number(data.costo_credito) : (existingConfig.costo_credito !== undefined ? Number(existingConfig.costo_credito) : 1000.00),
        comision_administrador: data.comision_administrador !== undefined ? Number(data.comision_administrador) : (existingConfig.comision_administrador !== undefined ? Number(existingConfig.comision_administrador) : 1500.00),
        comision_vendedor: data.comision_vendedor !== undefined ? Number(data.comision_vendedor) : (existingConfig.comision_vendedor !== undefined ? Number(existingConfig.comision_vendedor) : 800.00),
        comision_referente: data.comision_referente !== undefined ? Number(data.comision_referente) : (existingConfig.comision_referente !== undefined ? Number(existingConfig.comision_referente) : 200.00),
        ...
        try {
          ...
          for (const plan of data.sale_plans) {
            ...
            const fullPayload: any = {
              ...
              order: plan.order != null ? Number(plan.order) : 999,
              custom_override: !!plan.custom_override
            };
            ...
            try {
              ...
              console.warn("[saveIptvFinances] Upsert primario en iptv_planes_venta falló, intentando payload de contingencia:", err.message || err);
              try {
                // Contingencia sin columnas de categoría ni orden por si acaso
                ...
                // Fallback ultra seguro original básico
                const safePayload = {
                  id: plan.id,
                  provider_plan_id: plan.provider_plan_id || null,
                  name: plan.name,
          ...
          throw planErr; // Propagar error para que no sea un éxito falso
        }
      }
      ...
      console.warn("[saveIptvFinances] Error guardando finanzas en Supabase:", e.message || e);
      // Guardado local solo como contingencia silenciosa de memoria, pero informamos al cliente el fallo
