...
interface AuthContextType {
  ...
  profile: any | null; // Alias de compatibilidad para vistas
  isDevMode?: boolean;
  loading: boolean;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const loadUserRolesAndPerms = async () => {
      try {
        ...
        const datosAdicionales = profileData?.datos_adicionales || {};
        if (Array.isArray(profileData?.roles) && profileData.roles.length > 0) {
          userRolesList = profileData.roles;
        } else if (Array.isArray(datosAdicionales.roles) && datosAdicionales.roles.length > 0) {
          userRolesList = datosAdicionales.roles;
        ...
        } else if (typeof datosAdicionales.roles === 'string' && datosAdicionales.roles.startsWith('[')) {
          try {
            userRolesList = JSON.parse(datosAdicionales.roles);
          } catch {
            userRolesList = [profileData.rol || panelSession.rol || 'VENDEDOR'];
          }
        } else {
  ...
  const hasPermission = useCallback((node: string, actionType: 'ver' | 'interactuar' = 'ver'): boolean => {
    ...
    if (hasAnyIptvChild) {
      if (!normalizedPermissions.some(p => p.toLowerCase() === 'iptv.acceder')) {
        normalizedPermissions.push('Iptv.Acceder');
      }
    ...
    const isGranted = normalizedPermissions.some((p: string) => {
      ...
      // Equivalencias y unificaciones de alias para XTV
      // 1. Acceso General a XTV
      const xtvAccessNodes = [
        'iptv.acceder',
        'iptv.inicioresendores.ingresar',
        'iptv.iniciorevendedores.ingresar',
        'inicio.xtv.ver',
        'inicio.xtv.acceder',
        'iptv.inicioresendores.veryinteractuar',
        'iptv.iniciorevendedores.veryinteractuar'
      ];
      if (xtvAccessNodes.includes(cleanNode) && xtvAccessNodes.includes(cleanPerm)) {
        return true;
      }

      // 2. Creación directa
      if (
        (cleanNode === 'iptv.lineas.creardirecto' || cleanNode === 'iptv.creardirecto.ver' || cleanNode === 'iptv.creardirecto.acceder') &&
        (cleanPerm === 'iptv.lineas.creardirecto' || cleanPerm === 'iptv.creardirecto.ver' || cleanPerm === 'iptv.creardirecto.acceder')
      ) {
        return true;
      }
      ...
      if (
        (cleanNode === 'iptv.lineas.solicitar' || cleanNode === 'iptv.solicitaractivacion.ver' || cleanNode === 'iptv.solicitaractivacion.acceder') &&
        (cleanPerm === 'iptv.lineas.solicitar' || cleanPerm === 'iptv.solicitaractivacion.ver' || cleanPerm === 'iptv.solicitaractivacion.acceder')
      ) {
        return true;
      }
      ...
      if (
        (cleanNode === 'iptv.solicitudes.gestionar' || cleanNode === 'iptv.solicitudes.ver' || cleanNode === 'iptv.solicitudes.acceder' || cleanNode === 'iptv.solicitudes.aprobar') &&
        (cleanPerm === 'iptv.solicitudes.gestionar' || cleanPerm === 'iptv.solicitudes.ver' || cleanPerm === 'iptv.solicitudes.acceder' || cleanPerm === 'iptv.solicitudes.aprobar')
      ) {
        return true;
      }
      ...
      // 5. Clientes
      if (
        (cleanNode === 'iptv.clientes.ver' || cleanNode === 'iptv.clientes.acceder' || cleanNode === 'iptv.clientes.gestionar') &&
        (cleanPerm === 'iptv.clientes.ver' || cleanPerm === 'iptv.clientes.acceder' || cleanPerm === 'iptv.clientes.gestionar')
      ) {
        return true;
      }
      ...
      // 6. Finanzas
      if (
        (cleanNode === 'iptv.finanzas.ver' || cleanNode === 'iptv.finanzas.acceder' || cleanNode === 'iptv.finanzasrevendedores.ver' || cleanNode === 'iptv.finanzasrevendedores.acceder') &&
        (cleanPerm === 'iptv.finanzas.ver' || cleanPerm === 'iptv.finanzas.acceder' || cleanPerm === 'iptv.finanzasrevendedores.ver' || cleanPerm === 'iptv.finanzasrevendedores.acceder')
      ) {
        return true;
      }
