...
export function UserProfilesManagementTab({ 
  ...
  const loadUserDataIntoForm = (user: any) => {
    ...
    // Multi-roles
    let roles: string[] = [];
    const datosBancarios = user.negocio_datos_bancarios || user.datos_adicionales?.negocio_datos_bancarios || {};
    ...
    if (Array.isArray(user.roles) && user.roles.length > 0) {
      ...
      try {
        roles = JSON.parse(datosAdicionales.roles);
      } catch {
        roles = [user.rol || 'VENDEDOR'];
      }
    } else if (user.rol) {
  ...
  const handleSaveUserProfile = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const currentUserTarget = usersList.find(u => u.id === selectedUserId);
      const existingDatos = currentUserTarget?.datos_adicionales || {};

      const mergedDatosAdicionales = {
        ...existingDatos,
      ...
      const updatePayload: any = {
        ...
        telefono_contacto: formPersonal.telefono.trim(),
        avatar_url: formPersonal.avatar_url,
        rol: primaryRole,
        activo: !isBlocked,
        bloqueado: isBlocked,
        estado: isBlocked ? 'bloqueado' : 'activo',
        nombre_negocio: formG3d.business_name || formPersonal.nombre.trim(),
        direccion_negocio: formG3d.address || formPersonal.direccion.trim(),
        logo_url: formG3d.logo_url || formXtv.logo_url || '',
        datos_adicionales: mergedDatosAdicionales
      };
      ...
      setUsersList(prev => prev.map(u => u.id === selectedUserId ? {
        ...
        perfil_g3d: formG3d,
        perfil_xtv: formXtv,
        negocio_datos_bancarios: mergedDatosAdicionales.negocio_datos_bancarios
      } : u));
            ...
            {filteredUsers.length === 0 ? (
              ...
              filteredUsers.map(user => {
                ...
                const userRolesArr = (() => {
                  ...
                  if (Array.isArray(user.datos_adicionales?.roles) && user.datos_adicionales.roles.length > 0) return user.datos_adicionales.roles;
