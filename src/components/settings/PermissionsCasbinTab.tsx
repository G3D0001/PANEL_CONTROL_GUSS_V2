...
export function PermissionsCasbinTab({ currentUser, onDataChanged, initialSelectedUserId }: PermissionsCasbinTabProps) {
    ...
    try {
      ...
      if (dbUsers && dbUsers.length > 0) {
        ...
        if (initialSelectedUserId) {
          const target = dbUsers.find(u => u.id === initialSelectedUserId);
  ...
  const handleSelectUser = (user: any) => {
    ...
    let roles: string[] = [];
    const userDatos = user.datos_adicionales || {};
    if (Array.isArray(user.roles) && user.roles.length > 0) {
      roles = user.roles;
    } else if (Array.isArray(userDatos.roles) && userDatos.roles.length > 0) {
      roles = userDatos.roles;
    ...
    } else if (typeof userDatos.roles === 'string' && userDatos.roles.startsWith('[')) {
      try {
        roles = JSON.parse(userDatos.roles);
      } catch {
        roles = [user.rol || 'VENDEDOR'];
      }
    } else if (user.rol) {
  ...
  const handleSavePermissions = async () => {
    ...
    try {
      if (selectionType === 'role') {
        ...
        const currentTargetUser = usersList.find(u => u.id === selectedUserId);
        const existingDatos = currentTargetUser?.datos_adicionales || {};
        const updatedDatos = {
          ...existingDatos,
        ...
        const { error } = await supabase
          ...
          .update({
            ...
            datos_adicionales: updatedDatos
          })
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-[10px]">
                      <span className={`font-mono truncate ${isSelected ? 'text-indigo-200' : 'text-slate-500'}`}>
                        Rol: {Array.isArray(user.datos_adicionales?.roles) && user.datos_adicionales.roles.length > 1
                          ? `${user.datos_adicionales.roles.length} roles asignados`
                          : (roleObj?.nombre || user.rol || 'Sin rol')}
                      </span>
