...
export function SettingsView() {
  const { user, profile, userProfile, userRole, userRoles, hasPermission, isDevMode, refreshProfile } = useAuth();
  ...
  const activeRol = String(userRole || profile?.rol || userProfile?.rol || '').trim().toLowerCase();
  const isAdmin = activeRol === 'admin' || 
    activeRol === 'administrador' || 
    userRoles?.some(r => ['admin', 'administrador'].includes(String(r).trim().toLowerCase())) || 
    isDevMode || 
    hasPermission?.('Admin.*') ||
    hasPermission?.('*');
