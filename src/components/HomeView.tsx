...
export function HomeView() {
  ...
  const xtvMenuItems = [
    {
      id: 'crear_directo',
      title: customization.cards?.crear_directo?.title || 'Crear Línea Nueva',
      shortTitle: 'Alta Línea',
      description: 'Activación directa vía panel XC o solicitud mediante ticket.',
      ...
      action: () => {
        if (!hasPermission('Iptv.Acceder') && !hasPermission('Iptv.Lineas.CrearDirecto') && !hasPermission('Iptv.Lineas.Solicitar') && !hasPermission('Iptv.*') && !hasPermission('Admin.*')) {
          toast.warning("No tienes permiso de Acceso para esta función.");
          return;
        }
    ...
    if (item.id === 'crear_directo') return hasPermission('Iptv.Acceder') || hasPermission('Iptv.Lineas.CrearDirecto') || hasPermission('Iptv.Lineas.Solicitar') || hasPermission('Iptv.*') || hasPermission('Admin.*');
    if (item.id === 'renovaciones') return hasPermission('Iptv.Acceder') || hasPermission('Iptv.*') || hasPermission('Admin.*');
    if (item.id === 'finanzas') return hasPermission('Iptv.Acceder') || hasPermission('Iptv.Solicitudes.Gestionar') || hasPermission('Iptv.*') || hasPermission('Admin.*');
    if (item.id === 'mis_clientes') return hasPermission('Iptv.Acceder') || hasPermission('Iptv.Clientes.VerTodos') || hasPermission('Iptv.*') || hasPermission('Admin.*');
    if (item.id === 'finanzas_vendedores') return hasPermission('Iptv.Finanzas.Ver') || hasPermission('Iptv.*') || hasPermission('Admin.*');
    if (item.id === 'tutoriales') return hasPermission('Iptv.Tutoriales.Acceder') || hasPermission('Iptv.*') || hasPermission('Admin.*');
    if (item.id === 'ajustes_configuracion') return hasPermission('Iptv.Acceder') || hasPermission('Iptv.*') || hasPermission('Admin.*');
    return true;
  });
