...
export function Dashboard() {
  ...
  // Permisos granulares de creación de líneas y utilidades
  const canEditResellerNotes =
    isAdmin ||
    hasPermission("Iptv.Lineas.EditarNotasReseller") ||
    hasPermission("Iptv.Lineas.*") ||
    hasPermission("Iptv.*") ||
    hasPermission("Admin.*");

  const canUseTestAdmin =
    isAdmin ||
    hasPermission("Iptv.Lineas.TestAdmin") ||
    hasPermission("Iptv.Lineas.*") ||
    hasPermission("Iptv.*") ||
    hasPermission("Admin.*");
  ...
  const [loading, setLoading] = useState<boolean>(() => {
    // Si ya existen finanzas o cuentas en caché local, no bloqueamos la UI con pantalla completa
    try {
      const fin = localStorage.getItem('g3d_iptv_finanzas');
      if (fin) return false;
    } catch {}
    return true;
  });
  ...
  const [providerPlans, setProviderPlans] = useState<any[]>(() => {
    try {
      const stored = localStorage.getItem('g3d_iptv_finanzas');
      if (stored) return JSON.parse(stored).provider_plans || [];
    } catch {}
    return [];
  });
  const [salePlans, setSalePlans] = useState<any[]>(() => {
    try {
      const stored = localStorage.getItem('g3d_iptv_finanzas');
      if (stored) return JSON.parse(stored).sale_plans || [];
    } catch {}
    return [];
  });
  ...
  // --- GESTIÓN DE BORRADORES LOCALES DE CLIENTES ---
  const [savedDrafts, setSavedDrafts] = useState<any[]>(() => {
    try {
      return JSON.parse(localStorage.getItem("g3d_iptv_draft_clients") || "[]");
    } catch {
      return [];
    }
  });
  ...
  const handleLoadDraft = (draft: any) => {
    setDirectName(draft.nombre_completo || "");
    setDirectCelular(draft.celular || "");
    setDirectDireccion(draft.direccion_actual || "");
    if (draft.tipo_cuenta) setDirectType(draft.tipo_cuenta);
    if (draft.plan_id) setDirectPlanId(draft.plan_id);
    if (draft.pantallas) setDirectPantallas(draft.pantallas);
    if (draft.username) setDirectUser(draft.username);
    if (draft.password) setDirectPass(draft.password);
    if (draft.comprobante_url) setDirectComprobante(draft.comprobante_url);
    if (draft.notas_admin) setDirectNotes(draft.notas_admin);
    if (draft.reseller_notes) setDirectResellerNotes(draft.reseller_notes);
    toast.success(`📂 Borrador de "${draft.nombre_completo}" cargado al formulario.`);
  };

  const handleDeleteDraft = (draftId: string, clientName: string) => {
    const updated = savedDrafts.filter((d: any) => d.id !== draftId);
    setSavedDrafts(updated);
    localStorage.setItem("g3d_iptv_draft_clients", JSON.stringify(updated));
    toast.info(`Borrador de "${clientName}" eliminado.`);
  };
  ...
  const defaultLaunchpadCards = useMemo(() => [
    {
      id: "crear_directo",
      title: "Crear Línea Nueva",
      badge: "⚡ Alta de Servicio",
      iconName: "Sparkles",
      color1: "#10b981",
      color2: "#059669",
      action: () => {
        if (
          hasPermission("Iptv.Acceder") ||
          hasPermission("Iptv.Lineas.CrearDirecto") ||
          hasPermission("Iptv.Lineas.Solicitar") ||
          hasPermission("Iptv.*") ||
          hasPermission("Admin.*")
        ) {
          selectMenuWithScroll("crear_directo");
        } else {
          toast.error("Acceso Denegado: No tienes permisos asignados para dar de alta líneas.");
        }
      },
      showCondition:
        hasPermission("Iptv.Acceder") ||
    ...
    },
    {
      ...
      color2: "#2563eb",
      action: () => {
        if (hasPermission("Iptv.Acceder") || hasPermission("Iptv.*") || hasPermission("Admin.*")) {
          selectMenuWithScroll("renovaciones");
        } else {
          toast.error("Acceso Denegado: No tienes el permiso 'Iptv.Acceder' asignado.");
        }
      },
      showCondition: hasPermission("Iptv.Acceder") || hasPermission("Iptv.*") || hasPermission("Admin.*"),
    },
    {
      id: "finanzas",
      title: "Solicitudes",
      ...
      color2: "#7c3aed",
      action: () => {
        if (hasPermission("Iptv.Solicitudes.Gestionar") || hasPermission("Iptv.Solicitudes.Acceder") || hasPermission("Iptv.*") || hasPermission("Admin.*")) {
          selectMenuWithScroll("finanzas");
        } else {
          toast.error("Acceso Denegado: No tienes el permiso 'Iptv.Solicitudes.Gestionar' asignado.");
        }
      },
      showCondition: hasPermission("Iptv.Solicitudes.Gestionar") || hasPermission("Iptv.Solicitudes.Ver") || hasPermission("Iptv.Solicitudes.Acceder") || hasPermission("Iptv.*") || hasPermission("Admin.*"),
    },
    {
      id: "mis_clientes",
      title: "Mis Clientes",
    ...
    },
    {
      ...
      color2: "#db2777",
      action: () => {
        if (hasPermission("Iptv.Finanzas.Ver") || hasPermission("Iptv.Finanzas.Acceder") || hasPermission("Iptv.FinanzasRevendedores.Acceder") || hasPermission("Iptv.Finanzas.GestionarLiquidaciones") || hasPermission("Iptv.*") || hasPermission("Admin.*")) {
          selectMenuWithScroll("finanzas_vendedores");
        } else {
          toast.error("Acceso Denegado: No tienes el permiso 'Iptv.Finanzas.Ver' asignado.");
        }
      },
      showCondition: hasPermission("Iptv.Finanzas.Ver") || hasPermission("Iptv.Finanzas.Acceder") || hasPermission("Iptv.FinanzasRevendedores.Ver") || hasPermission("Iptv.Finanzas.GestionarLiquidaciones") || hasPermission("Iptv.*") || hasPermission("Admin.*"),
    },
    {
      id: "tutoriales",
  ...
  const isCardLocked = (cardId: string) => {
    ...
    if (cardId === "renovaciones") return !(hasPermission("Iptv.Acceder") || hasPermission("Iptv.*") || hasPermission("Admin.*"));
    if (cardId === "finanzas") return !(hasPermission("Iptv.Acceder") || hasPermission("Iptv.Solicitudes.Gestionar") || hasPermission("Iptv.*") || hasPermission("Admin.*"));
    if (cardId === "mis_clientes") return !(hasPermission("Iptv.Acceder") || hasPermission("Iptv.Clientes.VerTodos") || hasPermission("Iptv.*") || hasPermission("Admin.*"));
    if (cardId === "finanzas_vendedores") return !(hasPermission("Iptv.Finanzas.Ver") || hasPermission("Iptv.Finanzas.GestionarLiquidaciones") || hasPermission("Iptv.*") || hasPermission("Admin.*"));
    if (cardId === "ajustes_configuracion") {
      const hasConf =
        hasPermission("Admin.*") ||
        hasPermission("Iptv.*") ||
        hasPermission("Iptv.Acceder") ||
        hasPermission("Iptv.Clientes.VerTodos") ||
  ...
  // Carga inicial unificada de datos optimizada en paralelo
  const fetchData = async () => {
    try {
      // 1. Ejecución paralela de todas las consultas a base de datos
      const [usersDataRes, accsRes, financesRes, requestsRes, sysConfRes] = await Promise.allSettled([
        apiService.getIptvPanelUsers(),
        apiService.getIptvAccounts(),
        apiService.getIptvFinances(),
        apiService.getIptvCreditRequests(),
        apiService.getSystemConfig()
      ]);

      // Procesar usuarios y créditos
      let usersData: any[] = [];
      if (usersDataRes.status === 'fulfilled' && usersDataRes.value) {
        usersData = usersDataRes.value;
      ...
      // Procesar finanzas y planes
      let finances: any = null;
      if (financesRes.status === 'fulfilled' && financesRes.value) {
        finances = financesRes.value;
      ...
      if (requestsRes.status === 'fulfilled' && requestsRes.value) {
        const requests = requestsRes.value;
      ...
      if (sysConfRes.status === 'fulfilled' && sysConfRes.value) {
        sysConf = sysConfRes.value;
      ...
      // 2. Apagar indicador de carga de forma inmediata: la pantalla ya tiene todo listo
      setLoading(false);

      // 3. Sincronizar créditos reales del panel XC en segundo plano (asíncrono, no bloquea la UI)
      if (sysConf && sysConf.xui_url) {
        fetch("/api/iptv/xui", {
          ...
          .catch(err => {
            console.warn("Background XC credits check error:", err);
          });
      }
    } catch (err) {
  ...
  const handleRefreshDashboardCredits = async () => {
    ...
    try {
      ...
      if (xuiData.success) {
        ...
        let creditsVal = null;
        if (rawData.data?.credits !== undefined) creditsVal = Number(rawData.data.credits);
        else if (rawData.credits !== undefined) creditsVal = Number(rawData.credits);
        ...
        else if (rawResp.data?.credits !== undefined) creditsVal = Number(rawResp.data.credits);
        else if (rawResp.credits !== undefined) creditsVal = Number(rawResp.credits);
        else if (xuiData.data?.data?.credits !== undefined) creditsVal = Number(xuiData.data.data.credits);
    ...
    if (directType === "VIP") {
      ...
      const activeDemo = salePlans.filter(
        (p) =>
          !p.archived &&
          p.visible !== false &&
          (p.categoria_id === 'demo' || p.name.toLowerCase().includes("demo")),
      );
  ...
  useEffect(() => {
    if (selectedClientForDetails) {
      const selectablePlans = salePlans.filter(
        (p: any) => p.visible !== false && !p.name.toLowerCase().includes("demo") && p.categoria_id !== 'demo' && !p.archived,
      );
      
      const existingPlan = salePlans.find(
        (p: any) => p.id === selectedClientForDetails.id_plan_venta
      );
      const isDemo = existingPlan?.categoria_id === 'demo' || existingPlan?.name?.toLowerCase().includes("demo");
  ...
  const handleConfirmRenewal = async () => {
    const selectablePlans = salePlans.filter(
      (p: any) => p.visible !== false && !p.name.toLowerCase().includes("demo") && p.categoria_id !== 'demo',
    );
    const planToUse =
      selectedPlanForRenew ||
  ...
  const handleCreateDirectAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Validar créditos devueltos por el panel solo si es VIP (las pruebas demos son gratuitas en créditos)
      const rawData = testData.data?.data || testData.data || {};
      const xcCredits = Number(
        testData.credits ?? 
        rawData.credits ?? 
        testData.profile?.credits ?? 
        testData.data?.credits ?? 
        999
      );
      if (directType === "VIP" && xcCredits < 1) {
        toast.error(`❌ Paso 1 Falló: Créditos insuficientes en tu panel físico XC (Disponibles: ${xcCredits}). Se requiere al menos 1 crédito reseller para cuentas VIP.`);
        setDirectSubmitting(false);
        return;
      }
    } catch (err: any) {
    ...
    try {
      ...
      if (xuiData && xuiData.success) {
        ...
        if (resUpdate.success) {
          ...
          setDirectPass("");
          setDirectComprobante(null);
          // Actualización instantánea en memoria sin bloquear la pantalla con recarga completa
          setAccounts(prev => [updatedAccount, ...prev.filter(a => a.username !== updatedAccount.username)]);
        } else {
  ...
  // --- GUARDAR CLIENTE COMO BORRADOR ---
  const handleSaveDraftClient = () => {
    const clName = directName.trim();
    if (!clName) {
      toast.error("Por favor, ingresa al menos el Nombre del Cliente para guardar el borrador.");
      return;
    }
    try {
      const draftClient = {
        id: `draft_${Date.now()}`,
        nombre_completo: clName,
        celular: directCelular.trim(),
        direccion_actual: directDireccion.trim(),
        tipo_cuenta: directType,
        plan_id: directPlanId,
        pantallas: directPantallas,
        username: directUser.trim(),
        password: directPass.trim(),
        comprobante_url: directComprobante,
        notas_admin: directNotes.trim(),
        reseller_notes: directResellerNotes.trim(),
        creado_al: new Date().toISOString(),
        estado: "borrador"
      };

      const existingDrafts = JSON.parse(localStorage.getItem("g3d_iptv_draft_clients") || "[]");
      const updatedDrafts = [draftClient, ...existingDrafts.filter((d: any) => d.nombre_completo !== clName)];
      localStorage.setItem("g3d_iptv_draft_clients", JSON.stringify(updatedDrafts));
      setSavedDrafts(updatedDrafts);

      toast.success(`💾 Borrador de "${clName}" guardado correctamente.`);
    } catch (err: any) {
      toast.error("Error al guardar borrador: " + (err.message || err));
    }
  };
  ...
  const handleRequestActivation = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        apiService.getIptvCreditRequests().then((reqs) => setCreditRequests(reqs)).catch(() => {});
    ...
    try {
      ...
      if (res.success) {
        ...
        if (!isAdmin) {
          if (isVip) {
            const remaining = currentUserCreditsVIP - creditsCost;
            setCurrentUserCreditsVIP(remaining);
            setCurrentUserCredits(remaining);
            await apiService.updateIptvPanelUserCredits(
              user.email,
              remaining,
              currentUserCreditsDemo,
            ...
            setCurrentUserCreditsDemo(remaining);
            await apiService.updateIptvPanelUserCredits(
              user.email,
        ...
        // Actualización instantánea en memoria sin pantalla bloqueante
        setAccounts(prev => prev.map(a => a.username === updatedAccount.username ? updatedAccount : a));
      } else {
  ...
  const handleCreateDemo = async (e: React.FormEvent) => {
    ...
    if (systemConfig?.xui_url && systemConfig?.xui_token) {
      ...
      const apiSets = systemConfig?.api_settings || {};
      ...
      if (playlistId && !isNaN(Number(playlistId))) {
        payloadToSend.custom_playlist_id = Number(playlistId);
      }
    ...
    if (res.success) {
      ...
      // Limpiar form y actualizar lista local inmediatamente
      setDemoName("");
      setDemoCelular("");
      setDemoEmail("");
      setDemoNotes("");
      setAccounts(prev => [newAccount, ...prev.filter(a => a.username !== newAccount.username)]);
  ...
  const handleCreateVip = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      setAccounts(prev => [newVipAccount, ...prev.filter(a => a.username !== newVipAccount.username)]);
      setCurrentMenu("inicio");
    } else {
  ...
  return (
    <div className="px-5 py-6 md:py-10 w-full space-y-8 text-slate-900 dark:text-slate-100">
      ...
      {loading && providerPlans.length === 0 && salePlans.length === 0 && (
      ...
      {(!loading || providerPlans.length > 0 || salePlans.length > 0) && (
        <div id="active-widget-container" className="space-y-6">
          {/* A. WIDGET UNIFICADO: ALTA DE LÍNEA XTV (Crear Línea Nueva: Activar / Solicitar / Borrador) */}
          {(currentMenu === "crear_directo" || currentMenu === "solicitar_activacion") && (() => {
            const canCreateDirect = hasPermission("Iptv.Lineas.CrearDirecto") || hasPermission("Iptv.Acceder") || hasPermission("Iptv.*") || hasPermission("Admin.*");
            const canRequestLine = hasPermission("Iptv.Lineas.Solicitar") || hasPermission("Iptv.Acceder") || hasPermission("Iptv.*") || hasPermission("Admin.*");
              ...
              <div className="space-y-6 animate-fade-in">
                ...
                <div className="flex items-center justify-between">
                  <button
                    ...
                    className="px-4 py-2 text-xs font-bold text-slate-500 hover:text-slate-900 dark:hover:text-slate-200 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors flex items-center gap-1.5"
                  >
                    <ArrowLeft size={14} /> Volver al Inicio
                  </button>
                </div>

                {/* Grid Principal: Formulario a la Izquierda y Monitor/Resultado a la Derecha */}
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                  {/* Columna Izquierda: Formulario Unificado (7 cols) */}
                  <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6 shadow-sm">
                    ...
                    <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
                      <div className="flex items-center gap-3">
                        <div className="size-11 rounded-2xl flex items-center justify-center shrink-0 bg-cyan-500/10 text-cyan-500">
                          <Sparkles size={22} />
                        </div>
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] uppercase font-black px-2 py-0.5 rounded-md bg-emerald-100 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-300">
                              Alta de Servicio
                            </span>
                          </div>
                          <p className="text-slate-500 text-xs mt-0.5">
                            Completa los datos del cliente y elige la acción deseada al final del formulario.
                          </p>
                        </div>
                      </div>
                      ...
                      {canUseTestAdmin && (
                        <button
                          ...
                          className="px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-white font-extrabold text-[11px] uppercase tracking-wider rounded-xl transition-all shadow-md shadow-amber-500/10 flex items-center gap-1.5 shrink-0"
                          title="Rellenar datos de prueba para testing"
                        >
                          ⚡ Test (Admin)
                        </button>
                      )}
                    ...
                    {/* Banner de Corrección de Solicitud (si aplica) */}
                    {correctingRequestId && (
                      <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/50 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs text-amber-800 dark:text-amber-300 animate-fade-in">
                        <div className="space-y-1">
                          ...
                          <div className="text-slate-600 dark:text-slate-300">
                            ...
                            <strong>{directName || "(Sin nombre)"}</strong>.
                            {(() => {
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            setDirectPass("");
                            setDirectComprobante(null);
                            toast.info("Corrección cancelada. Formulario reiniciado.");
                          }}
                    ...
                    <form
                      onSubmit={(e) => {
                        e.preventDefault();
                        if (canCreateDirect) {
                          handleCreateDirectAccount(e);
                        } else {
                          handleRequestActivation(e);
                        }
                      }}
                      className="space-y-4"
                    >
                      ...
                      {/* Botones Selector Tipo de Cuenta (Demo vs VIP) */}
                      <div className="space-y-1">
                      ...
                      {directType && (
                        <div className="space-y-4 animate-fade-in mt-4">
                            ...
                            >
                              {directType === "VIP"
                                ? salePlans
                                    ...
                                    .map((p) => {
                                      const duracionText = p.months ? `${p.months} ${Number(p.months) === 1 ? 'mes' : 'meses'}` : p.hours ? `${p.hours} hs` : '1 mes';
                                      const precioText = p.price > 0 ? `$${Number(p.price).toLocaleString('es-AR')}` : 'Gratis';
                                      return (
                                        <option key={p.id} value={p.id}>
                                          {p.name} - {duracionText} - {precioText}
                                        </option>
                                      );
                                    })
                                : salePlans
                                    ...
                                    .map((p) => {
                                      const duracionText = p.hours ? `${p.hours} hs` : p.months ? `${p.months} ${Number(p.months) === 1 ? 'mes' : 'meses'}` : '2 hs';
                                      const precioText = p.price > 0 ? `$${Number(p.price).toLocaleString('es-AR')}` : 'Prueba Gratuita';
                                      return (
                                        <option key={p.id} value={p.id}>
                                          {p.name} - {duracionText} - {precioText}
                                        </option>
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-800/50 ring-1 ring-slate-200 dark:ring-slate-800 rounded-2xl px-4 py-3">
                              ...
                              <span className="text-sm font-bold text-slate-700 dark:text-slate-300">
                                {directPantallas} Pantalla{directPantallas > 1 ? "s" : ""} activa{directPantallas > 1 ? "s" : ""} en simultáneo
                              </span>
                            </div>
                          </div>

                          {/* Carga de Comprobante de Pago (Galería/Explorador - Regla #21) */}
                          {(() => {
                            ...
                            return (
                              <div className="space-y-2">
                                <label className="text-[10px] uppercase font-black text-slate-400 tracking-wider block">
                                  ...
                                  {isReqComprobante
                                    ? "(Requerido para Solicitar Activación)"
                                    : "(Opcional para Cuentas Gratuitas/Demo)"}
                                </label>
                                          ...
                                          try {
                                            ...
                                            reader.onload = (event) => {
                                              ...
                                              img.onload = () => {
                                                try {
                                                  ...
                                                  if (ctx) {
                                                    ...
                                                    toast.success("¡Captura optimizada correctamente (~40 KB)!");
                                                  } else {
                                                  ...
                                                  console.error("Error al renderizar canvas:", canvasErr);
                                                  setDirectComprobante(event.target?.result as string);
                                                  toast.dismiss(loadingToast);
                                              ...
                                              img.onerror = () => {
                                                setDirectComprobante(event.target?.result as string);
                                                toast.dismiss(loadingToast);
                                                toast.success("Comprobante cargado en formato original.");
                                              };
                          ...
                          <div className="space-y-1.5">
                            ...
                            <div className="relative">
                              <input
                                ...
                                readOnly={!canEditResellerNotes || !isResellerNotesEdited}
                                onChange={(e) => {
                                ...
                                className={`w-full rounded-2xl px-4 py-3 focus:outline-none text-sm font-semibold transition-all ${
                                  !canEditResellerNotes || !isResellerNotesEdited
                            ...
                            <p className="text-[10px] text-slate-400 leading-normal">
                              {canEditResellerNotes
                                ? isResellerNotesEdited
                                  ...
                                  : "Esta nota identifica al cliente en el panel físico XC y en los registros contables."
                                : "Esta nota se genera dinámicamente según el cliente, plan y tu usuario vendedor."}
                            </p>
                          </div>

                          {/* Notas Adicionales / Soporte */}
                          <div className="space-y-1">
                            <label className="text-[10px] uppercase font-black text-slate-400 tracking-wider">
                              Notas Adicionales o Mensaje al Soporte
                            </label>
                            <textarea
                              placeholder="Ej: Requiere canales de fútbol, detalles del pago o preferencias del cliente..."
                              value={directNotes}
                              rows={2}
                          ...
                          {/* Terna de Botones de Acción: Activar Línea, Solicitar Activación y Guardar en Borrador */}
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                            {/* 1. Activar Línea (Carga Directa XC) */}
                            {canCreateDirect && (
                              <button
                                type="button"
                                id="btn-accion-activar-linea"
                                disabled={directSubmitting}
                                onClick={(e) => {
                                  e.preventDefault();
                                  handleCreateDirectAccount(e as any);
                                }}
                                className="w-full py-3.5 px-4 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-600/50 text-white font-black rounded-2xl text-xs sm:text-sm transition-all shadow-md shadow-emerald-600/10 flex items-center justify-center gap-2 min-h-[48px]"
                              >
                                {directSubmitting ? (
                                  <>
                                  ...
                                  <>
                                    <Zap size={16} />
                                    <span>Activar Línea</span>
                                  </>
                                )}
                              </button>
                            )}

                            {/* 2. Solicitar Activación (Ticket con Comprobante) */}
                            {canRequestLine && (
                              <button
                                type="button"
                                id="btn-accion-solicitar-activacion"
                                disabled={directSubmitting}
                                onClick={(e) => {
                                  e.preventDefault();
                                  handleRequestActivation(e as any);
                                }}
                                className="w-full py-3.5 px-4 bg-amber-600 hover:bg-amber-700 disabled:bg-amber-600/50 text-white font-black rounded-2xl text-xs sm:text-sm transition-all shadow-md shadow-amber-600/10 flex items-center justify-center gap-2 min-h-[48px]"
                              >
                                {directSubmitting ? (
                                  <>
                                    <Loader2 className="animate-spin" size={16} />
                            ...
                            <button
                              type="button"
                              id="btn-accion-guardar-borrador"
                              onClick={handleSaveDraftClient}
                              className="w-full py-3.5 px-4 border-2 border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 font-black rounded-2xl text-xs sm:text-sm transition-all flex items-center justify-center gap-2 min-h-[48px]"
                              title="Guarda los datos completados para continuar más tarde"
                            >
                  ...
                  {/* Columna Derecha: Resultado o Información + Borradores Guardados (5 cols) */}
                  <div className="lg:col-span-5 space-y-6">
                    {/* Caso 1: Resultado de Cuenta Creada Exitosamente */}
                    {directResult ? (
                      <div className="bg-emerald-50 dark:bg-emerald-950/20 border-2 border-emerald-500/30 rounded-3xl p-6 sm:p-8 space-y-6 shadow-sm relative overflow-hidden animate-fade-in">
                        ...
                        <div>
                          ...
                          <p className="text-emerald-600 dark:text-emerald-500 text-xs mt-1">
                            Copia las credenciales abajo o mándalas directamente por WhatsApp al cliente.
                          </p>
                        </div>
                      ...
                      /* Caso 2: Panel de Información según la modalidad activa */
                      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 space-y-4 shadow-sm">
                        <div className="flex items-center gap-3">
                          <div className="size-10 rounded-xl flex items-center justify-center shrink-0 bg-cyan-500/10 text-cyan-500">
                            <Zap size={20} />
                          </div>
                          <div>
                            <h4 className="font-extrabold text-sm">
                              Guía de Alta de Líneas
                            </h4>
                            <p className="text-slate-500 text-xs">
                              Opciones de activación en el pie del formulario
                            </p>
                          </div>
                        </div>

                        <div className="space-y-2.5 text-xs text-slate-600 dark:text-slate-400">
                          <div className="flex items-start gap-2 bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
                            <span className="font-black text-emerald-600 dark:text-emerald-400 shrink-0">1.</span>
                            <span><strong>Activar Línea:</strong> Carga inmediata en el panel XC consumiendo tus créditos. Entrega las credenciales al instante.</span>
                          </div>
                          <div className="flex items-start gap-2 bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
                            <span className="font-black text-amber-600 dark:text-amber-400 shrink-0">2.</span>
                            <span><strong>Solicitar Activación:</strong> Crea un ticket con comprobante de pago para que un administrador autorice el alta y asigne tu comisión.</span>
                          </div>
                          <div className="flex items-start gap-2 bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
                            <span className="font-black text-slate-600 dark:text-slate-300 shrink-0">3.</span>
                            <span><strong>Guardar en Borrador:</strong> Almacena los datos completados localmente para recuperarlos con un clic cuando lo necesites.</span>
                          </div>

                          {(hasPermission("Iptv.Solicitudes.Acceder") || hasPermission("Iptv.Solicitudes.Gestionar") || hasPermission("Iptv.*") || hasPermission("Admin.*")) && (
                            <button
                              type="button"
                              onClick={() => selectMenuWithScroll("finanzas")}
                              className="w-full mt-2 py-2.5 px-4 bg-amber-50 hover:bg-amber-100 dark:bg-amber-950/30 dark:hover:bg-amber-950/50 text-amber-900 dark:text-amber-300 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-2 border border-amber-200 dark:border-amber-900/50"
                            >
                              <Inbox size={14} />
                    ...
                    {/* Módulo de Borradores Guardados */}
                    {savedDrafts.length > 0 && (
                      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 space-y-3 shadow-sm animate-fade-in">
                        ...
                        <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
                          {savedDrafts.map((draft) => (
                            <div
                              key={draft.id}
                              className="p-3 bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 rounded-2xl flex items-center justify-between gap-2 hover:bg-slate-100/80 dark:hover:bg-slate-800/80 transition-all"
                            >
                              <div className="min-w-0 flex-1">
                                <p className="text-xs font-bold truncate">
                                  {draft.nombre_completo}
                                </p>
                                <div className="flex items-center gap-2 text-[10px] text-slate-400 mt-0.5">
                                  <span className="font-semibold text-cyan-600 dark:text-cyan-400">
                                    {draft.tipo_cuenta || "VIP"}
                                  </span>
                                  {draft.celular && (
                                    <span className="truncate">📞 {draft.celular}</span>
                                  )}
                                </div>
                              </div>
                              ...
                              <div className="flex items-center gap-1.5 shrink-0">
                                <button
                                  ...
                                  className="px-2.5 py-1.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-[10px] font-black rounded-xl hover:opacity-90 transition-all"
                                ...
                                <button
                                  type="button"
                                  onClick={() => handleDeleteDraft(draft.id, draft.nombre_completo)}
                    ...
                    {(() => {
                      ...
                      return filteredVip.map((acc: any) => {
                        ...
                        const hasDirectPrivilege =
                          hasPermission("Iptv.Acceder") || isAdmin;
                              ...
                              <div className="bg-slate-950 text-slate-300 p-3.5 rounded-2xl text-[11px] space-y-1.5 border border-slate-850">
                                ...
                                <div className="flex justify-between">
                                  ...
                                  <strong className="text-amber-400 font-bold">
                                    ${Number(planToUse?.price || 0).toLocaleString('es-AR')} ARS
                                  </strong>
                                </div>
          ...
          {currentMenu === "finanzas_vendedores" &&
            (() => {
              const sellerEmailLower = (user?.email || "").toLowerCase().trim();
              const isUserAdmin = isAdmin;
              const canManageLiquidaciones = isUserAdmin || hasPermission("Iptv.Finanzas.GestionarLiquidaciones") || hasPermission("Iptv.Finanzas.Liquidar") || hasPermission("Iptv.*") || hasPermission("Admin.*");
                ...
                <div id="active-widget-container" className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    {canManageLiquidaciones ? (
                      <div className="bg-slate-900 text-white border border-slate-800 rounded-2xl p-5 shadow-sm space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Caja Global Liquidaciones</span>
                  ...
                  <div className="flex border-b border-slate-200 dark:border-slate-800 gap-1 overflow-x-auto pb-px">
                    ...
                    {canManageLiquidaciones && (
                      ...
                      >
                        ⚖️ Liquidar Comisiones {isUserAdmin ? "(Admin)" : ""}
                      </button>
                    )}
                  </div>
                  ...
                  {/* TAB 3: LIQUIDAR COMISIONES */}
                  {finanzasTab === "liquidaciones" && canManageLiquidaciones && (
