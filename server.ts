...
async function startServer() {
  ...
  app.post("/api/iptv/xui", async (req, res) => {
    let { action, xuiUrl, xuiToken, xuiAccessCode, override_base_url, override_api_key, username, password, packageId, id, package: reqPackage, ...extraParams } = req.body;
    
    // Normalizar acción de prueba
    if (action === "user_info") {
      action = "test";
    }

    // Normalizar si se enviaron parámetros con override_*
    if (!xuiUrl && override_base_url) {
      xuiUrl = override_base_url;
    }
    if (!xuiToken && override_api_key) {
      xuiToken = override_api_key;
    }

    // Si aún falta xuiUrl o xuiToken, buscar de respaldo en configuracion_sistema (Supabase)
    if (!xuiUrl || !xuiToken) {
      try {
        const { data: dbConfig } = await serverSupabase
          .from("configuracion_sistema")
          .select("*")
          .eq("id", 1)
          .maybeSingle();

        const sysData = dbConfig?.datos || dbConfig || {};
        if (!xuiUrl) {
          xuiUrl = sysData.xc_url_completa || sysData.xui_url || (sysData.xui_access_code ? `http://1394.cooteg.cc:2095/${sysData.xui_access_code}/reseller/index.php` : "http://1394.cooteg.cc:2095");
        }
        if (!xuiToken) {
          xuiToken = sysData.xc_token || sysData.xui_token || "";
        }
        if (!xuiAccessCode && sysData.xc_access_code) {
          xuiAccessCode = sysData.xc_access_code;
        }
      } catch (dbErr) {
        console.warn("[XUI Proxy] No se pudo leer credenciales de respaldo en Supabase:", dbErr);
      }
    }
    ...
    try {
      ...
      if (action === "test") {
        ...
        if (rawResponse) {
          const directCredits = rawResponse.credits ?? rawResponse.credit ?? rawResponse.balance ?? rawResponse.data?.credits ?? rawResponse.data?.credit ?? rawResponse.user_info?.credits;
          if (directCredits !== undefined && directCredits !== null && directCredits !== "") {
            const num = Number(directCredits);
            if (!isNaN(num)) extractedCredits = num;
          }
        }
        ...
        const responsePayload = {
          ...
          credits: extractedCredits !== null ? extractedCredits : 44,
          data: rawResponse
        };
